    ...
    internal class PostQuery : IPostQuery
    {
        ...
        public List<PostAdminQueryModel> GetAllPostsForAdmin()
        {
            return _postRepository.GetAsQueryable()
                ...
                {
                    Active = p.Active,
                    CreationDate = p.CreateDate.ConvertToPersianDate(),
                    Id = p.Id,
        ...
        public PostAdminDetailQueryModel GetPostDetails(int id)
        {
            ...
            var prices = _postPriceRepository.GetAsQueryable(p => p.PostId == post.Id);
            PostAdminDetailQueryModel model = new()
            {
                Active = post.Active,
                CityPricePlus = post.CityPricePlus,
                CreationDate = post.CreateDate.ConvertToPersianDate(),
